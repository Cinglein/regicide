...
pub type ActorList = Arc<RwLock<Vec<ActorId>>>;
                ...
                }) = self.users.get_mut(&user_id)
                {
                    ...
                    match send_sender.send(send_action.clone()) {
                        Ok(()) => {
                            ...
                            let actor_list = self.actors.keys().copied().collect();
                            let mut lock = self.actor_list.blocking_write();
                            *lock = actor_list;
                        }
                        Err(_err) => (),
                    }
...
pub struct Actor<A: Action> {
    ...
    server_msgs: HashMap<UserId, Sender<ServerMsg>>,
}

impl<A: Action> Actor<A> {
    fn spawn(user_id: UserId, send_server_msg: Sender<ServerMsg>) -> (Self, Sender<(A, UserId)>) {
...
pub enum JoinReq<A: Action> {
    Connect {
        ...
        send_server_msg: Sender<ServerMsg>,
...
pub trait Action: Clone + Debug + Serialize + DeserializeOwned + Send + Sync + 'static {
    type Shared: Default;
    type User: Default;
